...
function Register() {
  ...
  const [registerData, setRegisterData] = useState({
    ...
    email: "",
    password: "",
    roles: ['user'] // Cambiar 'admin' a 'user' por defecto
  })
  ...
  return (
    <div className={registerStyles.container}>
      <Stack spacing={2} className={registerStyles.card} justifyContent="center" alignItems="center">
        ...
        {/* Aquí puedes agregar un checkbox o algún otro selector para asignar el rol de admin, si lo deseas */}
        {/* Ejemplo de un selector de rol */}
        <div>
          <label>
            <input 
              type="checkbox" 
              checked={registerData.roles.includes('admin')} 
              onChange={(e) => {
                const updatedRoles = e.target.checked 
                  ? [...registerData.roles, 'admin'] 
                  : registerData.roles.filter(role => role !== 'admin');
